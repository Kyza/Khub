					...
					return class VoiceChannelScreenshare extends Plugin {
						...
						observer({ addedNodes }) {
							for (const node of addedNodes) {
								try {
									if (
										node.className.indexOf(KSS.createClassName("|containerDefault|")) > -1
									) {
										this.patch();
									}
								} catch (e) {}
						...
						patch(disconnect = false) {
							...
							for (let i = 0; i < channelElements.length; i++) {
								...
								let channelReactElement = ReactTools.getOwnerInstance(channelElement);
								if (channelReactElement.props.channel.type == 2) {
									if (disconnect && channelReactElement.props.connected) {
										let leaveButton = document.querySelector(`[name="Nova_CallLeave"]`)
											.parentElement.parentElement;
										leaveButton.click();
									}

									Patcher.after(channelReactElement, "handleVoiceConnect", () => {
										if (channelReactElement.props.connected) {
											WebpackModules.getByProps("selectChannel").selectChannel(
									...
									if (buttons) {
										if (!buttons.querySelector(".vcs-copy-button")) {
											...
											copyButton.setAttribute("aria-label", "Copy Channel Link");
