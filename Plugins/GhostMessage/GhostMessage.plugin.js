    ...
    const plugin = (Plugin, Api) => {
      ...
      return class GhostMessage extends Plugin {
        ...
        patch() {
          ...
          Patcher.after(DiscordModules.MessageActions, "_sendMessage", (thisObject, methodArguments, returnValue) => {
          ...
          Patcher.before(WebpackModules.getByProps("instantBatchUpload"), "upload", (thisObject, methodArguments) => {
            if (enabled) {
              let channel = DiscordAPI.currentChannel;
              files.push({
          ...
          Patcher.before(WebpackModules.getByProps("instantBatchUpload"), "instantBatchUpload", (thisObject, methodArguments) => {
            if (enabled) {
              let channel = DiscordAPI.currentChannel;
            ...
            if (event.type == "MESSAGE_CREATE") {
              for (const attachment of event.message.attachments) {
                let foundAttachment = files.find((file) => {
                  return file.name == attachment.filename && file.content == methodArguments[0].message.content && DiscordAPI.User.fromId(methodArguments[0].message.author.id).tag == DiscordAPI.currentUser.tag;
                });
