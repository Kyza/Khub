...
@if (@_jscript)
        ...
        // Offer to self-install for clueless users that try to run this
directly. var shell = WScript.CreateObject("WScript.Shell"); var fs = new
ActiveXObject("Scripting.FileSystemObject"); var pathPlugins =
shell.ExpandEnvironmentStrings("%APPDATA%\BetterDiscord\plugins"); var pathSelf
= WScript.ScriptFullName;
        // Put the user at ease by addressing them in the first person
        shell.Popup("It looks like you've mistakenly tried to run me directly.
\n(Don't do that!)", 0, "I'm a plugin for BetterDiscord", 0x30); if
(fs.GetParentFolderName(pathSelf) === fs.GetAbsolutePathName(pathPlugins)) {
                shell.Popup("I'm in the correct folder already.", 0, "I'm
already installed", 0x40); } else if (!fs.FolderExists(pathPlugins)) {
                shell.Popup("I can't find the BetterDiscord plugins folder.\nAre
you sure it's even installed?", 0, "Can't install myself", 0x10); } else if
(shell.Popup("Should I copy myself to BetterDiscord's plugins folder for you?",
0, "Do you need some help?", 0x34) === 6) { fs.CopyFile(pathSelf,
fs.BuildPath(pathPlugins, fs.GetFileName(pathSelf)), true);
                // Show the user where to put plugins in the future
                shell.Exec("explorer " + pathPlugins);
                shell.Popup("I'm installed!", 0, "Successfully installed",
0x40);
        }
        WScript.Quit();
...
var SafeEmbedGenerator = (() => {
  const config = {
    ...
    "changelog" : [
      // {
      // 	"title": "New Stuff",
      // 	"items": ["Changed the embed API to my own.", "Added this
      // changelog."]
      // }
      // ,
      // {
      ...
      // ,
      {
        ...
        "type" : "improved",
        "items" : [
          "Hey everyone! I'd just like to ask you to visit a website called teamtrees.org if you haven't already, that's all. Don't bug support about this, it's just an announcement."
        ]
      } // ,
        // {
        // 	"title": "In Progress",
  ...
  return !global.ZeresPluginLibrary ? class {
    ...
    load() {
      PluginUpdater.checkForUpdate(
          "SafeEmbedGenerator", this.getVersion(),
          "https://raw.githubusercontent.com/KyzaGitHub/Khub/master/Plugins/SafeEmbedGenerator/SafeEmbedGenerator.plugin.js");
    ...
    const plugin = (Plugin, Api) => {
      ...
      const {MessageStore, UserStore, ImageResolver, ChannelStore, Dispatcher} =
          DiscordModules;
      ...
      return class SafeEmbedGenerator extends Plugin {
        ...
        loadRecentEmbeds() {
          try {
            ...
            var fs = require("fs");
            recentEmbeds = JSON.parse(fs.readFileSync(
        ...
        addButton() {
          try {
            ...
            // Only add the button if the user has permissions to send messages
            // and embed links.
            if (this.isAllowed() && (this.hasPermission("textEmbedLinks") &&
                                     this.hasPermission("textSendMessages")) ||
                channel.type != "GUILD_TEXT") {
              if (document.getElementsByClassName("embed-button-wrapper")
                      .length == 0) {
                var daButtons =
                    document.getElementsByClassName("buttons-205you")[0];
                ...
                // class="icon-3D60ES da-icon" viewBox="0 0 22 22"
                // fill="currentColor"
                ...
                embedButtonIcon.setAttribute("style",
                ...
                embedButtonIcon.onmouseout = () => {
                  embedButtonIcon.setAttribute(
                      "style", "filter: invert(70%) !important;");
                };
                ...
                embedButton.onclick = () => {
                  ...
                  // Only send the embed if the user has permissions to embed
                  // links.
                  if (this.hasPermission("textEmbedLinks") ||
                      channel.type != "GUILD_TEXT") {
                    this.openEmbedPopup();
                  } else {
                    BdApi.alert(
        ...
        isAllowed() {
          ...
          if ([ '86004744966914048', '280806472928198656' ].includes(
                  guildId.toString())) {
            if (!betterDiscordServer1) {
              ...
              if (currentUser2.roles.length > 0) {
                ...
                // The user is in a BetterDiscord server but does not have a
                // role.
                return false;
              }
          ...
          // The user doesn't have any of the BetterDiscord servers selected, so
          // return true.
          return true;
        }

        removeButton() {
          if (document.getElementsByClassName("embed-button-wrapper").length >
              0) {
            document.getElementsByClassName("embed-button-wrapper")[0].remove();
          }
        }

        sendEmbed(providerName, providerUrl, authorName, authorUrl, title,
                  description, image, imageType, color) {
          var channelId = window.location.toString().split(
              "/")[window.location.toString().split("/").length - 1];
          var channel =
              DiscordAPI.Channel.from(DiscordAPI.Channel.fromId(channelId));
          ...
          if (this.isAllowed() && this.hasPermission("textEmbedLinks") ||
            ...
            obj.banner =
                (imageType == "true" ? true
                                     : false); // Photo is a banner, nothing is
                                               // a small image on the right.
            ...
            // var fs = require("fs");
            // fs.writeFile(pluginsFolder +
            // "/SafeEmbedGenerator.recentEmbeds.json",
            // JSON.stringify(recentEmbeds, null, 2), function(err) {
            //   if (err) {
            //     console.log(err);
            //   }
            // });
              ...
              json : obj
                    ...
                    (err, res, body) => {
                      ...
                      DiscordAPI.Channel.fromId(channelId).sendMessage(
                          `http://em.kyza.net/embed/${body.id}`, true);
                    });
          } else {
            BdApi.alert(
                "SafeEmbedGenerator",
        ...
        openEmbedPopup() {
          if (!document.getElementById("embedPopupWrapper")) {
            ...
            var embedButton =
                document.getElementsByClassName("embed-button-wrapper")[0]
            ...
            var positionInterval = setInterval(() => {
              ...
              popupWrapper.setAttribute(
                  ...
                  "text-align: center; border-radius: 10px; width: " +
                      popupWrapperWidth + "px; height: " + popupWrapperHeight +
            ...
            // Exit button: <svg width="18" height="18" class="button-1w5pas
            // da-button dropdown-33sEFX da-dropdown open-1Te94t da-open"><g
            // fill="none" fill-rule="evenodd"><path d="M0
            // 0h18v18H0"></path><path stroke="#FFF" d="M4.5 4.5l9 9"
            // stroke-linecap="round"></path><path stroke="#FFF" d="M13.5 4.5l-9
            // 9" stroke-linecap="round"></path></g></svg>
            var exitButton = document.createElement("div");
            exitButton.setAttribute(
                "style",
                "position: absolute; width: 18px; height: 18px; right: 10px; top: 10px;");
            exitButton.innerHTML =
                `<svg width="18" height="18" class="button-1w5pas dropdown-33sEFX open-1Te94t"><g fill="none" fill-rule="evenodd"><path d="M0 0h18v18H0"></path><path stroke="#FFF" d="M4.5 4.5l9 9" stroke-linecap="round"></path><path stroke="#FFF" d="M13.5 4.5l-9 9" stroke-linecap="round"></path></g></svg>`;
            ...
            providerName.setAttribute(
                "style", inputStyle + "margin-top: 10px;" + textInputStyle);
            providerName.oninput = () => {
              oldProviderName = providerName.value;

              this.createEmbedPreviewPopup(
                  popupWrapperWidth + 100, providerName.value,
                  providerUrl.value, authorName.value, authorUrl.value,
                  description.value, colorPicker.value,
                  imageTypeInput.getAttribute("checked"), imageUrl.value);
            };
            ...
            authorName.oninput = () => {
              if (authorName.value.trim() == "" &&
                ...
                this.disableUnusableInputs(authorName, description,
                                           providerName);
              } else {
                this.reenableNowUsableInputs(authorName, description,
                                             providerName);
              }
            ...
            imageType.onclick = () => {
              if (imageType.getAttribute("class") ==
            ...
            imageTypeInput.setAttribute(
                "class", "checkboxEnabled-CtinEn checkbox-2tyjJg");
            ...
            submitButton.onclick = () => {
              if (!(providerName.value.trim() == "" &&
                ...
                if ((providerName.value.trim() == "" &&
                     ...
                     imageUrl.value.trim() != "")) {
                  img.onload = function() {
                    this.sendEmbed(providerName.value, providerUrl.value,
                                   authorName.value, authorUrl.value, "",
                                   description.value, imageUrl.value,
            ...
            fadeOutBackground.setAttribute(
                ...
                `position: absolute; width: 100%; height: 100%;${
                    document.getElementsByClassName('platform-linux').length
                        ? ''
                        : ' top: 22px;'} background-color: rgba(0, 0, 0, 0.8); z-index: 999999999999999999998;`);
            fadeOutBackground.onclick = () => { this.closeEmbedPopup(); };

            document.body.appendChild(fadeOutBackground);

            // createRecentEmbedPopup(300);
            this.createEmbedPreviewPopup(
                popupWrapperWidth + 80, providerName.value, providerUrl.value,
                authorName.value, authorUrl.value, description.value,
                colorPicker.value, imageTypeInput.getAttribute("checked"),
                imageUrl.value);
          ...
          if (!document.getElementById("recentEmbedsWrapper")) {
            ...
            var positionInterval = setInterval(() => {
              ...
              recentEmbedsWrapper.setAttribute(
                  "style",
                  "border-radius: 10px; width: " + recentEmbedsWidth +
                      "px; height: " + recentEmbedsHeight +
                      "px; position: absolute; top: " +
                      ((window.innerHeight / 2) - (recentEmbedsHeight / 2) -
                       10) +
                      "px; left: " +
                      ((window.innerWidth / 2) - (recentEmbedsWidth / 2) -
                       offset) +
            ...
            recentEmbedsInner.setAttribute(
                "style", "overflow-y: scroll; width: " + recentEmbedsWidth +
                             "px; height: " + recentEmbedsHeight + "px;");
            ...
            for (var i = 0; i < recentEmbeds.length; i++) {
              ...
              embedElement.ondblclick = (e) => {
                ...
                var elementIndex =
                    parseInt(target.id.replace("embedNumber", ""));
        ...
        createEmbedPreviewPopup(offset, providerName, providerUrl, authorName,
                                authorUrl, description, color, imageType,
                                imageUrl) {
          var img = new Image();
          ...
          var create = (useImage, oldImage) => {
            ...
            if (imageType == "photo") {
              //			` + (imageType == "photo" ? `float:
              //right; max-width: 400px; max-height: 400px;` : `float: default;
              //max-width: 80px; max-height: 80px;`) + ("width: " + imageWidth +
              ...
              embedImageLink.setAttribute(
                  ...
                  "float: default; max-width: 400px; max-height: 400px; width: " +
                      scaledImageWidth + "px; height: " + scaledImageHeight +
                      "px;");
            ...
            var tmpString =
                ...
                (useImage ? (imageType == "thumbnail" ? imageString : "")
                          : "") +
            ...
            var htmlString =
                (providerName.trim() == "" && authorName.trim() == "" &&
                         description.trim() == "" && imageType == "photo"
                     ? (providerName.trim() == "" && authorName.trim() == "" &&
                                description.trim() == "" &&
                                imageUrl.trim() == ""
                            ? tmpString
            ...
            if (!document.getElementById("embedPreviewWrapper")) {
                ...
                previewWrapper.setAttribute(
                    ...
                    "border-radius: 10px; width: auto; height: auto; position: absolute; top: " +
                        ((window.innerHeight / 2) -
                         (previewWrapperHeight / 2)) +
                        "px; left: " +
                        ((window.innerWidth / 2) - (previewWrapperWidth / 2) +
                         offset) +
                        "px; background-color: #36393F; z-index: 999999999999999999999; padding: 10px; text-rendering: optimizeLegibility;");
              }, 100);
              ...
              var previewWrapper =
                  document.getElementById("embedPreviewWrapper");
              previewWrapper.innerHTML = htmlString;
              return previewWrapper;
            }
          };
          ...
          if (oldImageUrl != imageUrl) {
            img.onload = function() { create(true, false); };
            img.onerror = function() { create(false, false); };
        ...
        testImage(imageUrl) { return (imageUrl.trim() != "" ? true : false); }
        ...
        hasPermission(permission) {
          ...
          // General
          if (permission == "generalCreateInstantInvite")
            hexCode = 0x1;
          if (permission == "generalKickMembers")
            hexCode = 0x2;
          if (permission == "generalBanMembers")
            hexCode = 0x4;
          if (permission == "generalAdministrator")
            hexCode = 0x8;
          if (permission == "generalManageChannels")
            hexCode = 0x10;
          if (permission == "generalManageServer")
            hexCode = 0x20;
          if (permission == "generalChangeNickname")
            hexCode = 0x4000000;
          if (permission == "generalManageNicknames")
            hexCode = 0x8000000;
          if (permission == "generalManageRoles")
            hexCode = 0x10000000;
          if (permission == "generalManageWebhooks")
            hexCode = 0x20000000;
          if (permission == "generalManageEmojis")
            hexCode = 0x40000000;
          if (permission == "generalViewAuditLog")
            hexCode = 0x80;
          // Text
          if (permission == "textAddReactions")
            hexCode = 0x40;
          if (permission == "textReadMessages")
            hexCode = 0x400;
          if (permission == "textSendMessages")
            hexCode = 0x800;
          if (permission == "textSendTTSMessages")
            hexCode = 0x1000;
          if (permission == "textManageMessages")
            hexCode = 0x2000;
          if (permission == "textEmbedLinks")
            hexCode = 0x4000;
          if (permission == "textAttachFiles")
            hexCode = 0x8000;
          if (permission == "textReadMessageHistory")
            hexCode = 0x10000;
          if (permission == "textMentionEveryone")
            hexCode = 0x20000;
          if (permission == "textUseExternalEmojis")
            hexCode = 0x40000;
          // Voice
          if (permission == "voiceViewChannel")
            hexCode = 0x400;
          if (permission == "voiceConnect")
            hexCode = 0x100000;
          if (permission == "voiceSpeak")
            hexCode = 0x200000;
          if (permission == "voiceMuteMembers")
            hexCode = 0x400000;
          if (permission == "voiceDeafenMembers")
            hexCode = 0x800000;
          if (permission == "voiceMoveMembers")
            hexCode = 0x1000000;
          if (permission == "voiceUseVAD")
            hexCode = 0x2000000;
