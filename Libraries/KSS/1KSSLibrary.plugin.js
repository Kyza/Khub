...
window.KSSCache = {
	...
	addSelector: (selector, value) => {
		if (!window.KSSCache.selectors) {
			window.KSSCache.selectors = {};
		}
		window.KSSCache.selectors[selector] = value;
	},
	getSelector: (selector) => {
		if (window.KSSCache.selectors) {
			return window.KSSCache.selectors[selector];
		}
		return null;
	},
	clear: (cacheName) => {
		if (!cacheName) {
			let caches = Object.keys(window.KSSCache);
			for (let i = 0; i < caches.length; i++) {
				let cache = caches[i];
				if (typeof window.KSSCache[cache] != "function") {
					window.KSSCache[cache] = null;
				}
			}
		} else {
      window.KSSCache[cacheName] = null;
    }
	}
};
	...
	this.parse = (kss) => {
		...
		if (parsed) {
			for (const i of parsed) {
				if (window.KSSCache.getSelector(i)) {
					kss = kss.replace(i, window.KSSCache.getSelector(i));
				} else {
					...
					if (!res || !res[search[search.length - 1]]) {
						...
						try {
							...
							// Add the selector to the cache.
							window.KSSCache.addSelector(i, result);
						} catch (e) {}
					}
				}
			}
...
function toggleOverlay() {
	if (
		document.querySelector("#KSSOverlay").className.indexOf("hideKSSOverlay") < 0
	) {
	...
	return !global.ZeresPluginLibrary
		...
		: (([Plugin, Api]) => {
				const plugin = (Plugin, Api) => {
					const { Modals, PluginUpdater } = Api;
